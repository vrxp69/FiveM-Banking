...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if inRange and not shown then
        	shown = true
        	if Config.crptTextUI then
        		exports['crptTextUI']:Open('[E] To access the bank', 'darkblue', 'left')
            else
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if NearATM() and not isBankOpened and NearATM() ~= "update" then
            ...
            if IsControlJustReleased(0, 38) then
            	TriggerEvent("crptBanking:OpenATM")
            end
        ...
        if inRange and not shown then
        	shown = true
        	if Config.crptTextUI then
        		exports['crptTextUI']:Open('[E] To access the ATM', 'darkblue', 'left')
        	else
...
RegisterNetEvent("crptBanking:OpenATM")
AddEventHandler("crptBanking:OpenATM", function()
	local dict = 'anim@amb@prop_human_atm@interior@male@enter'
	local anim = 'enter'
	local ped = GetPlayerPed(-1)

	QBCore.Functions.TriggerCallback("crptBanking:GetPIN", function(pin)
		if pin then
			...
			exports['crptNotify']:Alert("BANK", "Head up to a bank to set a PIN code", 5000, 'info')
		end
	end)
end)
...
function openBank()
	...
	QBCore.Functions.TriggerCallback("crptBanking:GetPlayerInfo", function(data)
		QBCore.Functions.TriggerCallback("crptBanking:GetOverviewTransactions", function(cb, identifier, allDays)
			...
			if canAccessSociety then
				QBCore.Functions.TriggerCallback("crptBanking:SocietyInfo", function(cb)
					if cb ~= nil then
						societyInfo = cb
					else
						local societyIban = Config.IBANPrefix..jobLabel
						TriggerServerEvent("crptBanking:CreateSocietyAccount", society, jobLabel, 0, societyIban)
						Citizen.Wait(200)
						QBCore.Functions.TriggerCallback("crptBanking:SocietyInfo", function(cb)
							societyInfo = cb
						end, society)
			...
			QBCore.Functions.TriggerCallback("crptBanking:GetSocietyTransactions", function(societyTranscb, societyID, societyAllDays)
...
RegisterNUICallback("action", function(data, cb)
		...
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				if data.window == 'bankmenu' then
					TriggerServerEvent('crptBanking:DepositMoney', tonumber(data.value))
				elseif data.window == 'societies' then
					TriggerServerEvent('crptBanking:DepositMoneyToSociety', tonumber(data.value), societyInfo.society, societyInfo.society_name)
				end
			else
				exports['crptNotify']:Alert("BANK", "Invalid amount", 5000, 'error')
			end
		else
			exports['crptNotify']:Alert("BANK", "Invalid input", 5000, 'error')
		end
	elseif data.action == "withdraw" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				if data.window == 'bankmenu' then
					TriggerServerEvent('crptBanking:WithdrawMoney', tonumber(data.value))
				elseif data.window == 'societies' then
					TriggerServerEvent('crptBanking:WithdrawMoneyToSociety', tonumber(data.value), societyInfo.society, societyInfo.society_name, societyInfo.value)
				end
			else
				exports['crptNotify']:Alert("BANK", "Invalid amount", 5000, 'error')
			end
	...
	elseif data.action == "transfer" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				QBCore.Functions.TriggerCallback("crptBanking:IsIBanUsed", function(isUsed, isPlayer, name)
					if isUsed ~= nil then
						if data.window == 'bankmenu' then
							if isPlayer then
								TriggerServerEvent('crptBanking:TransferMoney', tonumber(data.value), data.iban:upper(), isUsed.citizenid, isUsed.money, name)
							elseif not isPlayer then
								TriggerServerEvent('crptBanking:TransferMoneyToSociety', tonumber(data.value), isUsed.iban:upper(), isUsed.society_name, isUsed.society)
							end
						elseif data.window == 'societies' then
							...
							if isPlayer then
								TriggerServerEvent('crptBanking:TransferMoneyToPlayerFromSociety', tonumber(data.value), data.iban:upper(), isUsed.citizenid, isUsed.money, name, societyInfo.society, societyInfo.society_name, societyInfo.value, toMyself)
							elseif not isPlayer then
								TriggerServerEvent('crptBanking:TransferMoneyToSocietyFromSociety', tonumber(data.value), isUsed.iban:upper(), isUsed.society_name, isUsed.society, societyInfo.society, societyInfo.society_name, societyInfo.value)
							end
						end
					elseif isUsed == nil then
						exports['crptNotify']:Alert("BANK", "This IBAN does not exist", 5000, 'error')
					end
				end, data.iban:upper())
			else
				exports['crptNotify']:Alert("BANK", "Invalid amount", 5000, 'error')
	...
	elseif data.action == "change_iban" then
		if Config.CustomIBANAllowLetters then
			...
			QBCore.Functions.TriggerCallback("crptBanking:IsIBanUsed", function(isUsed, isPlayer)

				if isUsed == nil then
					TriggerServerEvent('crptBanking:UpdateIbanDB', iban, Config.IBANChangeCost)
				elseif isUsed ~= nil then
					exports['crptNotify']:Alert("BANK", "This IBAN is already in use", 5000, 'error')
				end
			end, iban)
		elseif not Config.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				exports['crptNotify']:Alert("BANK", "You can only use numbers on your IBAN", 5000, 'error')
			end
		end
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				QBCore.Functions.TriggerCallback("crptBanking:GetPIN", function(pin)
            		if pin then
            			TriggerServerEvent('crptBanking:UpdatePINDB', data.pin, Config.PINChangeCost)
            		else
            			TriggerServerEvent('crptBanking:UpdatePINDB', data.pin, 0)
					end
				end)
			else
				exports['crptNotify']:Alert("BANK", "Your PIN needs to be 4 digits long", 5000, 'info')
			end
		else
			exports['crptNotify']:Alert("BANK", "You can only use numbers", 5000, 'info')
		end
	end
end)

RegisterNetEvent("crptBanking:updateTransactions")
AddEventHandler("crptBanking:updateTransactions", function(money, wallet)
	Citizen.Wait(100)
	if isBankOpened then
		QBCore.Functions.TriggerCallback("crptBanking:GetOverviewTransactions", function(cb, id, allDays)
			trans = cb
			walletMoney = wallet
			...
			TriggerEvent('crptBanking:updateMoney', money, wallet)
		end)
	end
end)

RegisterNetEvent("crptBanking:updateMoney")
AddEventHandler("crptBanking:updateMoney", function(money, wallet)
	if isBankOpened then
		playerBankMoney = money
		walletMoney = wallet
		SendNUIMessage({
...
RegisterNetEvent("crptBanking:updateIban")
AddEventHandler("crptBanking:updateIban", function(iban)
	playerIBAN = iban
	SendNUIMessage({
		action = 'updateiban',
		iban = playerIBAN,
...
RegisterNetEvent("crptBanking:updateIbanPinChange")
AddEventHandler("crptBanking:updateIbanPinChange", function()
	Citizen.Wait(100)
	QBCore.Functions.TriggerCallback("crptBanking:GetOverviewTransactions", function(cbs, ids, allDays)
		trans = cbs
	end)
end)

RegisterNetEvent("crptBanking:updateTransactionsSociety")
AddEventHandler("crptBanking:updateTransactionsSociety", function(wallet)
	Citizen.Wait(100)
	QBCore.Functions.TriggerCallback("crptBanking:SocietyInfo", function(cb)
		QBCore.Functions.TriggerCallback("crptBanking:GetSocietyTransactions", function(societyTranscb, societyID, societyAllDays)
			QBCore.Functions.TriggerCallback("crptBanking:GetOverviewTransactions", function(cbs, ids, allDays)
...
function GenerateIBAN()
	...
	while true do
		...
		if isIBanUsed and not hasChecked then
			isIBanUsed = false
			QBCore.Functions.TriggerCallback("crptBanking:IsIBanUsed", function(isUsed)
				if isUsed ~= nil then
					isIBanUsed = true
	...
	TriggerServerEvent('crptBanking:SetIBAN', iban)
end